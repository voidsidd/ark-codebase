...
app.post(
  ...
  async (req, res) => {
    ...
    const pyProcess = spawn(
      pythonExe,
      ['process_frame.py', tempPath, cameraId, zone || 'UNKNOWN'],
      {
        cwd: __dirname,
    ...
    pyProcess.on('close', (code) => {
      ...
      if (code !== 0) {
        console.error('[AI Engine] Process exited with code: ' + code + ' | Stderr: ' + stderr);
      ...
      try {
        ...
        if (result && result.error) {
          console.error('[AI Engine] Internal Script Error: ' + result.error);
        ...
        if (result && result.assessment) {
          ...
          processEvent({
            ...
            description:
              (assessment.verification && assessment.verification.reasoning) ||
              'AI verified threat signature detected.',
...
const ACOUSTIC_ACTIONS = {
  ...
  CHAINSAW:
    'Possible illegal logging. Notify forestry staff and deploy patrol to triangulated coordinates.',
  VEHICLE:
...
const FALLBACK_SPECIES = [
  ...
  },
  {
    ...
    statusLabel: 'VU',
    baseDirective:
      'Rhino presence confirmed. Cross-check with rhino monitoring team for collar or ear-notch ID.',
  },
  {
    label: 'African Lion',
...
app.post('/api/analyze/vision', async (req, res) => {
    ...
    try {
      ...
      if (data.status && data.status.code === 10000) {
        ...
        return res.status(200).json({
          ...
          endangered: end,
          statusLabel: 'LIVE',
          directive: `Machine Intelligence classified subject as ${pn} with ${(
            topPrediction.value * 100
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (top && (top.label || top.score !== undefined)) {
          ...
          const labelStr =
            (label && (typeof label === 'string' ? label : label.class || label.label)) ||
            'Wildlife';
          ...
          return res.status(200).json({
            success: true,
            classification: String(labelStr)
              .replace(/_/g, ' ')
  ...
  if (image) {
    console.log(
      '[Logic Engine] Using local heuristic fallback for classification. Set CLARIFAI_PAT or HF_TOKEN for live AI.'
    );
    const base64Data = image.replace(/^data:image\/\w+;base64,/, '');
...
app.post('/api/recommend-action', async (req, res) => {
  ...
  const fallback =
    fallbacks[alertType?.toUpperCase()] ||
    `Review alert in ${z} and deploy patrol if threat is confirmed.`;
...
function getLunarIllumination(lon = 0) {
  ...
  const phase =
    ((localNow.getTime() - KNOWN_NEW_MOON) % (29.53059 * 24 * 60 * 60 * 1000)) /
    (29.53059 * 24 * 60 * 60 * 1000);
...
app.get('/api/environment/:lat/:lon', async (req, res) => {
  ...
  try {
    const resp = await fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,wind_speed_10m,precipitation_probability,weather_code&timezone=auto`
    );
    const data = await resp.json();
    const cur = data.current;
    const lun = getLunarIllumination(parseFloat(lon));
    const threat = computeThreatMultiplier(
      lun,
      cur.wind_speed_10m,
...
app.get('/api/eonet/:lat/:lon', async (req, res) => {
  ...
  try {
    ...
    const nearby = (data.events || []).filter((e) => {
      ...
      return (
        coords &&
        Math.abs(coords[1] - parseFloat(lat)) < 5 &&
        Math.abs(coords[0] - parseFloat(lon)) < 5
      );
    });
...
app.get('/api/gbif/:lat/:lon', async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(
      `https://api.gbif.org/v1/occurrence/search?decimalLatitude=${parseFloat(lat) - 0.5},${parseFloat(lat) + 0.5}&decimalLongitude=${parseFloat(lon) - 0.5},${parseFloat(lon) + 0.5}&limit=5&hasCoordinate=true&basisOfRecord=HUMAN_OBSERVATION&year=2024,2025&kingdomKey=1`
    );
    const data = await response.json();
...
const ESTATE_COORDS = {
  ...
  corbett: { lat: 29.53, lon: 78.7747, radius: 45 },
    ...
    const results = (data.results || [])
      ...
      .map((obs, i) => {
        ...
        const timestamp = obs.observed_on
          ? new Date(obs.observed_on).toLocaleString('en-IN', {
              month: 'short',
              day: 'numeric',
              hour: '2-digit',
              minute: '2-digit',
...
app.post('/api/webhooks/purge-selected', (req, res) => {
  ...
  console.log(
    `[Admin] Selective purge: removed ${removed} alert(s) for estate ${estateId || 'all'}`
  );
...
function scheduleSim(minMs, maxMs, generator) {
  ...
  setTimeout(() => {
    try {
      ...
      console.error('[SIM] Error in simulation generator:', e);
    }
  ...
  const events = [
    {
      ...
      description: 'Acoustic sensor detected high-caliber discharge pattern in restricted sector.',
    },
    {
      subType: 'CHAINSAW',
      confidence: 0.84 + Math.random() * 0.12,
      description: 'Motorized cutting signature detected — possible illegal logging activity.',
    },
    {
      subType: 'VEHICLE_ENGINE',
      confidence: 0.8 + Math.random() * 0.12,
      description: 'Unscheduled vehicle engine signature recorded in restricted zone.',
    },
  ];
  ...
  const events = [
    {
      ...
      description: 'Camera trap identified animal presence in patrol sector.',
      priority: 'ELEVATED',
    },
    {
      subType: 'HUMAN_PRESENCE',
      confidence: 0.82 + Math.random() * 0.14,
      description: 'Unidentified human figure detected by camera trap in restricted area.',
      priority: 'HIGH',
    },
    {
      ...
      description:
        'Unusual animal behavioral pattern detected — possible stress or external disturbance.',
      priority: 'ELEVATED',
    },
  ];
  const e = simRandom(events);
  ...
  const events = [
    {
      ...
      description: 'Community member reported unidentified vehicle near estate boundary.',
    },
    {
      subType: 'SNARE_DETECTED',
      description: 'Active wire snare line reported by local community patrol member.',
    },
    {
      subType: 'DEAD_ANIMAL',
      description: 'Animal carcass discovered — possible poaching or disease event.',
    },
    {
      subType: 'POACHER_CAMP',
      description: 'Evidence of recent illegal encampment found by community scout.',
    },
  ];
  const e = simRandom(events);
  processEvent({
...
scheduleSim(30 * 1000, 30 * 1000, async () => {
  try {
    ...
    const resp = await fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${coords.lat}&longitude=${coords.lon}&current=temperature_2m,wind_speed_10m,precipitation_probability,weather_code&timezone=auto`
    );
    const data = await resp.json();
    const cur = data.current;
