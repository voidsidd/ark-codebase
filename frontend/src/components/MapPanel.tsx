...
const ICONS = {
  ...
  WILDLIFE_CORRELATION: createCustomIcon(
    '#10B981',
...
const FALLBACK_ICON = createCustomIcon(
  '#6B7280',
...
const MapPanel: React.FC<MapPanelProps> = ({ estateId, estateBoundary }) => {
  ...
  if (estateBoundary) {
    ...
    const positions: [number, number][] =
      estateBoundary.boundary?.coordinates[0]?.map(([lng, lt]) => [lt, lng] as [number, number]) ??
      [];
    ...
    <div className="w-full h-full relative" id="map-container">
              ...
              >
                <Popup className="vanguard-popup">
                  <div className="text-xs">
                    <strong className="text-white block text-sm border-b border-gray-700 pb-1 mb-1">
                      Zone {zoneId.replace('Z', '')}
                    </strong>
                    <span className="text-gray-400">Status: </span>
                    <span style={{ color }}>{status}</span>
                    {isVulnerable && (
                      <div className="mt-1 text-red-500 font-bold border-t border-gray-700 pt-1">
      ...
      <div className="absolute top-14 right-4 z-[400] bg-vanguard-panel/90 border border-vanguard-border backdrop-blur p-2 rounded text-xs flex flex-col gap-2 shadow-xl">
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-vanguard-zoneClear"></div>
          <span>Clear</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-vanguard-zoneMonitor"></div>
          <span>Monitoring</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-vanguard-zoneActive"></div>
        ...
        {predictiveState && (
          <div className="flex items-center gap-2 border-t border-gray-700 pt-2 mt-1">
            ...
            <span className="text-red-400 font-bold">
              Predictive Threat ({(predictiveState.lunarIllumination * 100).toFixed(0)}% Moon)
            </span>
          </div>
        )}
      </div>
