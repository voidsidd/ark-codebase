...
const getIcon = (type: string) => {
  switch (type) {
    ...
    case 'CORRELATED':
      return <ShieldAlert size={16} className="text-red-600 animate-pulse" />;
    case 'ONE_HEALTH':
...
const getAlertTheme = (type: string) => {
  switch (type) {
    ...
    default:
      return {
        border: 'border-vanguard-border',
        bg: 'bg-vanguard-panel',
        text: 'text-gray-400',
        icon: 'text-gray-500',
      };
  }
        ...
        {sortedAlerts.map((alert) => {
          ...
          return (
            <div
              ...
              className={`p-3 rounded border text-sm flex gap-3 transition-all duration-80 ease-out hover:bg-vanguard-hover cursor-pointer ${theme.bg} ${theme.border} ${theme.pulse ? 'animate-pulse' : ''}`}
            >
                  ...
                  <div className="flex items-center gap-2 flex-wrap min-w-0">
                    {isPredictive ? (
                      <PredictiveAlertBadge
                        correlationPct={alert.confidence ? Math.round(alert.confidence * 100) : 0}
                      />
                    ) : (
                      <span className={`font-bold text-[13px] ${theme.text}`}>
                    ...
                    {hasNarrative && (
                      <span
                        ...
                        className="text-[10px] font-bold tracking-widest bg-indigo-500/20 border border-indigo-500/40 text-indigo-400 rounded px-1.5 py-0.5"
                      >
                        AI
                      </span>
                    )}
