...
export default function LoadingScreen({ isLoading }: { isLoading: boolean }) {
  ...
  useEffect(() => {
    if (isLoading) {
      ...
      t1.current = setTimeout(() => {
        ...
        raf.current = requestAnimationFrame(() => requestAnimationFrame(() => setOpacity(1)));
  ...
  const transition =
    phase === 'entering'
      ? `opacity ${FADE_IN_MS}ms cubic-bezier(0.5, 0, 0.75, 0)`
      : phase === 'exiting'
        ? `opacity ${FADE_OUT_MS}ms cubic-bezier(0.7, 0, 1, 0.6)`
        : 'none';
