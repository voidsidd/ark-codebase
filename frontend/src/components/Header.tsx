...
const Header: React.FC<HeaderProps> = ({
  ...
  const estateDisplay = resolvedEstate
    ? resolvedEstate.name
    : estateId
      ? estateId.replace(/-/g, ' ').toUpperCase()
      : 'NAGARHOLE ESTATE';

  return (
    <header
      className="h-16 shrink-0 z-50 relative"
      style={{
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(90deg,transparent 0%,rgba(16,185,129,0.5) 35%,rgba(16,185,129,0.5) 65%,transparent 100%)',
        }}
      />

      {/* 3-column grid — center never overlaps left or right */}
      <div
        className="h-full grid items-center px-4 gap-2"
      ...
      >
        ...
        <div className="flex items-center justify-end gap-3 min-w-0 overflow-hidden pr-2">
          <button
            onClick={() => estateId && navigate(`/estate/${estateId}/cameras`)}
            title="Camera Feeds"
          ...
          <button
            onClick={onSpeciesIntel ?? (() => estateId && navigate(`/estate/${estateId}/species`))}
            title="Species ID"
            className="flex items-center gap-2 px-3.5 py-2 text-[11px] font-mono font-bold bg-vanguard-panel border border-vanguard-border rounded hover:border-vanguard-species/50 hover:text-vanguard-species transition-all shrink-0"
          >
          ...
          <button
            onClick={() => estateId && navigate(`/estate/${estateId}/sound`)}
