...
const CameraFeedsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/inaturalist/${id}`)
      .then((r) => (r.ok ? r.json() : []))
      .then((data) => {
        const photos: InatPhoto[] = (Array.isArray(data) ? data : []).map(
          (obs: {
            imageUrl: string;
            speciesCommonName: string;
            scientificName: string;
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <div className="px-6 pt-5 pb-4 border-b border-vanguard-border bg-vanguard-panel flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {loadingPhotos && (
              <span className="text-[9px] font-mono text-green-500/60 animate-pulse">
                SYNCING iNaturalist…
              </span>
            )}
          </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            {cameras.map((cam) => (
              <div
                key={cam.id}
                className="bg-vanguard-panel border border-vanguard-border rounded-lg overflow-hidden flex flex-col"
              >
                {/* Camera Header */}
                ...
                <div className="p-3 bg-vanguard-bg border-t border-vanguard-border flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 text-[9px] font-bold font-mono bg-vanguard-species/10 text-vanguard-species border border-vanguard-species/30 rounded hover:bg-vanguard-species hover:text-white transition-colors"
                    >
