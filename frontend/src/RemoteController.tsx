...
const RemoteController: React.FC = () => {
  ...
  const triggerAlert = useCallback(async (payload: TriggeredAlert) => {
      ...
      const { error } = await supabase.from('alerts').insert([
        {
          ...
          priority:
            payload.confidence && payload.confidence >= 0.9
              ? 'CRITICAL'
              : payload.confidence && payload.confidence >= 0.8
                ? 'HIGH'
                : 'ELEVATED',
    ...
    try {
      ...
      setIntelBrief({
        ...
        recommended_actions: [
          'Dispatch nearest ranger unit.',
          'Cross-reference adjacent sensor feeds.',
          'Log in Daily Intelligence Report.',
        ],
        risk_window:
          (payload.confidence ?? 0) >= 0.9 ? 'Immediate — next 2 hours' : 'Next 6–12 hours',
        classification:
          (payload.confidence ?? 0) >= 0.9
            ? 'CRITICAL'
            : (payload.confidence ?? 0) >= 0.8
              ? 'HIGH'
              : 'ELEVATED',
      });
    } finally {
  ...
  const getAlertTypeIcon = (type: string) => {
    switch (type) {
      case 'ACOUSTIC':
        return <Radio size={12} className="text-red-400" />;
      case 'CAMERA':
        return <Camera size={12} className="text-amber-400" />;
      case 'COMMUNITY':
        return <Users size={12} className="text-blue-400" />;
      default:
        return <ShieldAlert size={12} className="text-gray-400" />;
    }
  };
  ...
  const acousticButtons = [
    {
      ...
      payload: {
        ...
        subType: 'GUNSHOT',
        confidence: 0.95,
        description: 'High-caliber rifle discharge detected in sector.',
      },
    },
    {
      label: 'CHAINSAW',
      ...
      payload: {
        ...
        subType: 'CHAINSAW',
        confidence: 0.92,
        description: 'Motorized cutting signature — possible illegal logging.',
      },
    },
    {
      label: 'VEHICLE',
      ...
      payload: {
        ...
        zone: selectedZone,
        type: 'ACOUSTIC',
        subType: 'VEHICLE_ENGINE',
        confidence: 0.87,
        description: 'Unscheduled vehicle engine detected in restricted zone.',
      },
    },
  ];
  ...
  const visionButtons = [
    {
      ...
      payload: {
        ...
        confidence: 0.88,
        description: 'Unauthorized individual detected by camera trap.',
      },
    },
    {
      label: 'VEHICLE',
      ...
      payload: {
        ...
        zone: selectedZone,
        type: 'CAMERA',
        subType: 'VEHICLE_DETECTED',
        confidence: 0.85,
        description: 'Suspicious motor vehicle detected on camera.',
      },
    },
    {
      label: 'ANOMALY',
      ...
      payload: {
        ...
        zone: selectedZone,
        type: 'CAMERA',
        subType: 'BEHAVIORAL_ANOMALY',
        confidence: 0.82,
        description: 'Unusual animal behavioral pattern — possible stress or injury.',
      },
    },
  ];
  ...
  const communityButtons = [
    {
      ...
      payload: {
        ...
        zone: selectedZone,
        type: 'COMMUNITY',
        subType: 'SNARE_DETECTED',
        confidence: 0.78,
        description: 'Active wire snare line reported by community member.',
      },
    },
    {
      label: 'CAMP FOUND',
      ...
      payload: {
        ...
        zone: selectedZone,
        type: 'COMMUNITY',
        subType: 'POACHER_CAMP',
        confidence: 0.8,
        description: 'Evidence of recent illegal encampment discovered.',
      },
    },
    {
      label: 'DEAD ANIMAL',
      ...
      payload: {
        ...
        subType: 'DEAD_ANIMAL',
        confidence: 0.72,
        description: 'Dead wildlife reported — possible poaching or disease.',
      },
    },
  ];
  ...
  const renderButton = (btn: any) => {
    ...
    return (
      ...
      >
        ...
        <span className={`text-[9px] font-mono font-bold tracking-widest ${c.text}`}>
          {btn.label}
        </span>
      </button>
    );
    ...
    <div className="min-h-screen bg-[#05080F] text-gray-200 font-sans p-4 flex flex-col select-none overflow-x-hidden">
      ...
      <div className="flex items-center justify-between mb-5 pt-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-syne font-bold tracking-tighter text-white">
              VANGUARD REMOTE
            </h1>
        ...
        <div
          className={`px-3 py-1 rounded-full border transition-all duration-300 flex items-center gap-1.5 ${
            status.type === 'success'
        ...
        >
          <div
            className={`w-1.5 h-1.5 rounded-full ${status.type === 'success' ? 'bg-green-500 animate-pulse' : 'bg-current opacity-30'}`}
          />
      ...
      {/* Estate Selector */}
      <div className="mb-4">
        <label className="block text-[10px] font-mono text-gray-500 mb-2 tracking-widest font-bold">
          TARGET BATTLEGROUND
        </label>
      ...
      <div className="mb-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {acousticButtons.map((btn) => renderButton(btn))}
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {visionButtons.map((btn) => renderButton(btn))}
        </div>
      </div>
      ...
      <div className="mb-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          {communityButtons.map((btn) => renderButton(btn))}
        </div>
      </div>
        ...
        <div className="mb-5 border border-vanguard-primary/20 rounded-xl overflow-hidden bg-vanguard-primary/4">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-vanguard-primary/10 border-b border-vanguard-primary/15">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-mono font-bold tracking-widest text-vanguard-primary">
                ALERT INTELLIGENCE
              </span>
            </div>
            <div className="flex gap-2 items-center">
              ...
              <button
                ...
                className="text-gray-500 hover:text-white text-xs leading-none w-5 h-5 flex items-center justify-center"
              >
                ✕
              </button>
            </div>
          ...
          <div className="p-4">
            ...
            <div className="flex gap-2 mb-3 flex-wrap">
              <span className="text-[9px] font-mono bg-white/5 border border-white/10 text-gray-400 px-2 py-1 rounded">
                {lastTriggered.type}
              </span>
              <span className="text-[9px] font-mono bg-white/5 border border-white/10 text-gray-400 px-2 py-1 rounded">
                {lastTriggered.subType.replace(/_/g, ' ')}
              </span>
              <span className="text-[9px] font-mono bg-white/5 border border-white/10 text-gray-400 px-2 py-1 rounded">
                ZONE {lastTriggered.zone}
              </span>
              <span className="text-[9px] font-mono bg-white/5 border border-white/10 text-gray-400 px-2 py-1 rounded">
                {estate?.name}
              </span>
            ...
            {intelLoading && (
              <div className="flex items-center gap-2 py-2">
                ...
                <span className="text-[11px] font-mono text-vanguard-primary/70">
            ...
            {intelBrief && (
              <div className="space-y-3">
                ...
                {/* Actions */}
                <div>
                  <div className="text-[9px] font-mono text-gray-500 tracking-widest uppercase mb-1.5">
                    Recommended Actions
                  </div>
                  <ul className="space-y-1">
                    {intelBrief.recommended_actions.map((a, i) => (
                      <li key={i} className="flex gap-2 text-[11px] text-gray-400">
                        <span className="text-vanguard-primary shrink-0">▸</span>
                        {a}
                      </li>
                    ))}
                  </ul>
        ...
        {selectivePurgeOpen && (
          <div className="p-3 bg-[#080D18] border-t border-white/10">
            <div className="text-[9px] font-mono text-gray-500 mb-3 tracking-widest">
              SHOWING ALERTS FOR:{' '}
              <span className="text-orange-400">{estate?.name?.toUpperCase()}</span>
            </div>
            <div className="flex items-center justify-between mb-3 gap-2">
              ...
              >
                {selectedAlertIds.size === liveAlerts.length && liveAlerts.length > 0
                  ? 'DESELECT ALL'
                  : 'SELECT ALL'}
              </button>
              <button
            ...
            <div className="max-h-64 overflow-y-auto space-y-1.5 no-scrollbar">
              ...
              {!loadingAlerts && liveAlerts.length === 0 && (
                <div className="text-[10px] font-mono text-gray-600 text-center py-4">
                  NO ACTIVE ALERTS FOR THIS ESTATE
                </div>
              )}
              {!loadingAlerts &&
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          ...
                          <span className="text-[9px] font-mono text-gray-500 ml-auto shrink-0">
                            {alert.zone}
                          </span>
                        </div>
      ...
      <div className="mt-auto pb-4">
        <button
          onClick={() => {
            ...
            fetch('/api/webhooks/clear', { method: 'POST' }).catch(() => {});
            setStatus({ type: 'success', message: 'SYSTEM PURGE SENT' });
            setTimeout(() => setStatus({ type: null, message: '' }), 2000);
          }}
        ...
        >
          ...
          <span className="text-xs font-mono font-bold tracking-[0.2em] text-gray-400 group-active:text-red-400">
            PURGE ENTIRE SYSTEM FEED
          </span>
        </button>
      </div>
